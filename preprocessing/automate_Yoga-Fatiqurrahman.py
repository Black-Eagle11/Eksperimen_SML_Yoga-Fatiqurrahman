...
def balance_dataset(
    ...
    logging.info(f"Distribusi awal kelas: {dist_before} (rasio ≈ {imbalance_ratio:.3f})")
...
>>>>>>> 6775961c14cf2615fa9599c519d09c48a6978247
        ...
        logging.warning(f"Gagal melakukan parallel fillna, fallback ke mode normal: {e}")
        num_cols_all = df.select_dtypes(include=np.number).columns.tolist()
        cat_cols_all = [c for c in df.columns if c not in num_cols_all]

        if num_cols_all:
            df[num_cols_all] = df[num_cols_all].fillna(
                df[num_cols_all].median()
            )
